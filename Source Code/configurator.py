...
class Config(Tk):
    def __init__(self, *args, **kwargs):
        ...
        vulnerability_settings.update({"Main Menu": {"Style": StringVar(), "Desktop Checkbox": IntVar(), "Desktop Entry": StringVar(), "Silent Mode": IntVar(), "Server Mode": IntVar(), "Server Name": StringVar(), "Server User Name": StringVar(), "Server Password": StringVar(), "Tally Points": StringVar()},
                                  ...
                                  "Policy Options": {"Disable Auto Login": {"Definition": 'Remove all users from auto login. (lightdm)',
                                                     ...
                                                     "Turn On Firewall": {"Definition": 'Enable this to score the competitor for turning on the firewall. (ufw)',
                                                                          "Enabled": IntVar(),
                                                                          "Categories": {'Points': [IntVar()]}},
                                                     "Monitor Ports": {"Definition": '(WIP)List of ports that need to be open or closed.',
                                                                       "Enabled": IntVar(),
                                                                       "Categories": {'Points': [IntVar()]}}},
        ...
        for i, t in enumerate(vulnerability_settings["Password Policy"].keys()):
            self.add_option(LocalPolicyPageIn, vulnerability_settings["Password Policy"][t], t, i + 2, nb)
            l = i + 3
        ...
        for i, t in enumerate(vulnerability_settings["Policy Options"].keys()):
            self.add_option(LocalPolicyPageIn, vulnerability_settings["Policy Options"][t], t, i + l + 1, nb)
            ...
            if entry["Categories"]["Points"][i] != 0:
                for r, t in enumerate(entry["Categories"]):
                    if t == "Points":
                        ...
                        ttk.OptionMenu(modifyPageIn, entry["Categories"][t][i], *["running", "dead"]).grid(row=i + 3, column=r, sticky=EW)
                        c = r + 1
                    else:
                        ttk.Entry(modifyPageIn, textvariable=entry["Categories"][t][i]).grid(row=i + 3, column=r, sticky=EW)
                        c = r + 1
...
def add_row(frame, entry, widgets, default_row):
    ...
    for i, t in enumerate(entry):
        if t == "Points":
            ...
            ttk.OptionMenu(frame, entry[t][rwl], *["running", "dead"]).grid(row=tempr, column=i, sticky=EW)
            c = i + 1
        else:
            ttk.Entry(frame, textvariable=entry[t][rwl]).grid(row=tempr, column=i, sticky=EW)
            c = i + 1
    ...
    if vulnerability_settings["Forensic"]["Enabled"].get() == 1:
        for i, q in enumerate(vulnerability_settings["Forensic"]["Categories"]["Question"]):
            if q != 0 and q.get() != '':
                ...
                os.chmod((str(vulnerability_settings["Main Menu"]["Desktop Entry"].get()) + 'Forensic Question ' + str(i + 1) + '.txt'), 0o777)
...
def commit_config():
    ...
    shutil.copy('Scoring_Engine_Logo_Linux_Icon.png', os.path.join(output_directory, 'Scoring_Engine_Logo_Linux_Icon.png'))
    shutil.copy('save_data.json', os.path.join(output_directory, 'save_data.json'))
    shutil.copy('scoring_engine', os.path.join(output_directory, 'scoring_engine'))
    os.chmod(os.path.join(output_directory, 'scoring_engine'), 0o777)
    with open('start_scoring_engine', 'w+') as f:
        f.write('#!/bin/bash \n./scoring_engine')
    shutil.copy('start_scoring_engine', os.path.join(output_directory, 'start_scoring_engine'))
    os.chmod(os.path.join(output_directory, 'start_scoring_engine'), 0o777)
    with open('/etc/crontab', 'r') as f:
        if "/usr/local/CyberPatriot/scoring_engine" not in f.read():
            with open('/etc/crontab', 'r') as g:
                g.write('@reboot         root    /usr/local/CyberPatriot/scoring_engine')
    sys.exit()
    ...
    if save_dictionary["Account Management"]["User Change Password"]["Enabled"] == 1:
        ...
        for c in content:
            for name in save_dictionary["Account Management"]["User Change Password"]["Categories"]['User Name']:
                if name.lower() in c:
                    temp = c.split(':')
                    password_settings.update({name.lower(): temp[1]})
        filename = 'password_settings.json'
        with open(filename, 'w+') as f:
            json.dump(password_settings, f)
        f.close()
...
root.ttkStyle.set_theme(vulnerability_settings["Main Menu"]["Style"].get())
root.ttkStyle.theme_settings(themename="aquativo", settings={".": {"configure": {"background": '#eff0f1'}}, "TNotebook": {"configure": {"tabmargins": [2, 5, 2, 0]}}, "TNotebook.Tab": {"configure": {"width": int(root.winfo_screenwidth() * 3 / 4 / 7), "anchor": 'center'}}, "TLabel": {"configure": {"padding": '5 0', "justify": 'center', "wraplength": int(root.winfo_screenwidth() * 3 / 4 - 150)}}, "TEntry": {"map": {"fieldbackground": [('disabled', '#a9acb2')]}}, "TButton": {"configure": {"anchor": 'center', "width": '13'}}})
...
root.ttkStyle.theme_settings(themename="clearlooks", settings={"TNotebook": {"configure": {"tabmargins": [2, 5, 2, 0]}}, "TNotebook.Tab": {"configure": {"width": int(root.winfo_screenwidth() * 3 / 4 / 7), "anchor": 'center'}}, "TLabel": {"configure": {"padding": '5 0', "justify": 'center', "wraplength": int(root.winfo_screenwidth() * 3 / 4 - 180)}}, "TEntry": {"map": {"fieldbackground": [('disabled', '#b0aaa4')]}}, "TButton": {"configure": {"anchor": 'center', "width": '13'}}})
